...
function editarPágina(filePath, args, folder) {
  ...
  try {
    ...
    const emailedit = rolagrossona.replace(/\b[A-Za-z0-9._%+-]+@[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/g, `${args.get('email')}`)
    const numberedit = emailedit.replace(/(?:\+?\d{1,3}\s?)?(?:\(?\d{2}\)?\s?)?\d{4,5}[-\s]?\d{4}/g, `${edits['#telefone'].replace(/(\d{2})(\d{5})(\d{4})/, `($1) $2-$3`)}`)
